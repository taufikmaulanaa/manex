...
//debug($dtx_core2018);die;
	...
	foreach($produk[$m0->product_line] as $m2 => $m1) { 
		...
		<tr>
			...
			$sub_total_budget += $m1->total_budget;
	...
	<tr>
		...
		<?php
			...
			for ($i = 1; $i <= 12; $i++) {
				$sub_totalfield0 = 'sub_TotalB_' . sprintf('%02d', $i);
				echo '<td style="background: '.$bgedit.'"><div style="background:'.$bgedit.'" style="min-height: 10px; width: 50px; overflow: hidden;" contenteditable="'.$contentedit.'" class="edit-value text-right budget " data-name="" data-id="'.$m1->id.'" data-value="'.$$totalfield0.'"><b>'.number_format($$sub_totalfield0).'</b></td>';
			}
			echo '<td style="background: '.$bgedit.'"><div style="background:'.$bgedit.'" style="min-height: 10px; width: 50px; overflow: hidden;" contenteditable="'.$contentedit.'" class="edit-value text-right calculate" data-name="" data-id="'.$m1->id.'" data-value="'.$sub_total_budget.'"><b>'.number_format($sub_total_budget).'</b></td>';
...
<tr>
	...
	<?php
		...
		echo '<td style="background: '.$bgedit.'"><div style="background:'.$bgedit.'" style="min-height: 10px; width: 50px; overflow: hidden;" contenteditable="'.$contentedit.'" class="edit-value text-right calculate" data-name="" data-id="'.$m1->id.'" data-value="'.$stotal_budge.'"><b>'.number_format($stotal_budget).'</b></td>';
