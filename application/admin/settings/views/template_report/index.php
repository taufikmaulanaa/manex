	...
	table_open('',true);
		thead();
			tr();
				...
				th(lang('description'),'','width="450px"');
				th(lang('sum_of'),'text-center');
...
<?php 
	modal_open('modal-form','','modal-lg');
		modal_body();
			form_open(base_url('settings/template_report/save'),'post','form');
				col_init(3,9);
				input('hidden','id','id');
				...
				select2(lang('sum_of'),'sum_of[]','',$opt_acc,'account_code','account_name','','multiple');
...
<script type="text/javascript">
	function getData() {
		...
		$.ajax({
			url 	: base_url + 'settings/template_report/data',
			data 	: {},
			type	: 'get',
			dataType: 'json',
	...
	$('.btn-sort').click(function(){
		...
		$.ajax({
			url : base_url + 'settings/template_report/data/sortable',
			type : 'get',
			dataType : 'json',
			success : function(response) {
	...
	$('#save-posisi').click(function(e){
		...
		$.ajax({
			url : base_url + 'settings/template_report/save_sortable',
			type : 'post',
			data : serialized,
			dataType : 'json',
			success : function(response) {
